...
26. No settings UI, `data.json`, or code path references `knownContexts`, `enableMultipleNextActions`, `getContexts`/`parseContextList`, `ContextEditorSuggest`, or `src/editor/context-suggest.ts` — the whole Context/Multiple-Next-Actions feature was fully removed (see the Inline Field Format note above); a vault file with a pre-existing `[context:: ...]` inline field still renders as clean task text everywhere (generic inline-field stripping), it's just no longer a recognized concept
...
32. The Tasks Summary tab renders exactly three sections — Projects, Waiting, Inbox — with a priority filter and collapsible sections (see items 44-45) but no Context filter (removed)
