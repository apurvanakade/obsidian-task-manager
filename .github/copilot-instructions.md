...
src/
  ...
  review/
    ...
    weekly-review-data.ts        ← Collects Waiting/Someday-Maybe staleness rows; stamps the `reviewed` frontmatter field
```
...
An on-demand main-panel `ItemView` (`src/review/weekly-review-view.ts`), deliberately not a persistent sidebar leaf (unlike the date dashboard) and not a generated markdown note (unlike the two summary notes) — it needs interactive per-row actions that a static file or always-visible panel don't fit well, and it's meant to be a deliberate, occasional activity.
...
- `src/review/weekly-review-data.ts` is the pure(ish) data layer (one write function, `stampReviewedDate()`, otherwise read-only):
  - `collectWaitingReviewRows()` scans `settings.waitingProjectsFolder` via `isInFolder()` (`summary-file-io.ts`), reads `WAITING_SINCE_FRONTMATTER_FIELD` (imported from `task-processor.ts`, the field's owner/writer) via `readFrontmatterField()`, and computes `daysWaiting` and `isNewlyStale` (via `crossesThresholdWithinCurrentWeek()`, `date-utils.ts`, parameterized by `settings.waitingStalenessThresholdDays`).
  - `collectSomedayReviewRows()` scans `settings.somedayMaybeProjectsFolder`, reads `REVIEWED_FRONTMATTER_FIELD` (`"reviewed"`, owned by this module), and computes `daysSinceReview` and `needsReview` (never reviewed, or past `settings.somedayMaybeReviewCadenceDays`).
  - Both sort most-stale-first; rows with no timestamp at all (`daysWaiting`/`daysSinceReview === null`) sort first of all, ahead of any numeric staleness — implemented by treating `null` as `Number.POSITIVE_INFINITY` in the comparator.
...
- The Someday-Maybe table's **Mark Reviewed** button calls `stampReviewedDate()` directly then invokes the view's `onNeedsRefresh` callback (passed into `renderContent()`), which re-renders in place — no command re-run needed.
- `runOpenRandomSomedayMaybeProject()` in `main.ts` also calls `stampReviewedDate()` before opening the file, so the existing random-picker doubles as a lightweight review mechanism at no extra UI cost — this was a deliberate design choice, not an accidental side effect.
